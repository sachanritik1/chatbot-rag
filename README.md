...
> In this template, we use `@chatbot-rag` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@chatbot-rag` to something like `@my-company` or `@project-name`.
...
pnpm --filter @chatbot-rag/auth generate
```
...
3. Outputs a Drizzle-compatible schema file to the `@chatbot-rag/db` package
